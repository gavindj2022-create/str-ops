-- STR Ops D1 schema. This mirrors migrations/0001_initial.sql for a fresh database.
...
CREATE TABLE IF NOT EXISTS properties (
  ...
  cleaning_rate_cents INTEGER NOT NULL DEFAULT 0,
  ical_urls TEXT
);
...
CREATE TABLE IF NOT EXISTS login_attempts (
  ...
  window_started_ts TEXT NOT NULL,
...
CREATE TABLE IF NOT EXISTS turn_checks (
  ...
  photo_key TEXT,
  updated_by TEXT REFERENCES team(id) ON DELETE SET NULL,
  updated_ts TEXT,
...
CREATE TABLE IF NOT EXISTS water_assets (
  id TEXT PRIMARY KEY,
  property_id TEXT NOT NULL REFERENCES properties(id) ON DELETE CASCADE,
  type TEXT NOT NULL CHECK (type IN ('pool', 'hottub')),
  name TEXT NOT NULL
);
...
CREATE TABLE IF NOT EXISTS maintenance_tickets (
  ...
  opened_by TEXT REFERENCES team(id) ON DELETE SET NULL,
  note TEXT NOT NULL,
  photo_key TEXT,
...
CREATE TABLE IF NOT EXISTS financials (
  ...
  cleaning_cost_cents INTEGER NOT NULL DEFAULT 0,
  note TEXT,
...
CREATE TABLE IF NOT EXISTS goals (
  ...
  status TEXT NOT NULL DEFAULT 'active'
    CHECK (status IN ('active', 'hit', 'archived'))
);
...
CREATE TABLE IF NOT EXISTS photo_objects (
  ...
  uploaded_by TEXT REFERENCES team(id) ON DELETE SET NULL,
...
CREATE TABLE IF NOT EXISTS audit_log (
  id TEXT PRIMARY KEY,
  actor_id TEXT REFERENCES team(id) ON DELETE SET NULL,
